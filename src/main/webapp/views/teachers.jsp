...
<head>
    ...
    <title>PurpleWest - Teachers</title>
...
<div x-data="usePopper({placement:'right-end',offset:12})" @click.outside="isShowPopper &amp;&amp; (isShowPopper = false)" class="flex">
               ...
               <c:if test="${sessionScope.ROLE eq 'Admin' }">
                           <img class="rounded-full" src="https://upload.wikimedia.org/wikipedia/commons/thumb/1/12/User_icon_2.svg/1200px-User_icon_2.svg.png" alt="avatar">
                       </c:if> 
                        <c:if test="${sessionScope.ROLE ne 'Admin' }">
                           <img class="rounded-full" src="https://media.istockphoto.com/id/1433039224/photo/blue-user-3d-icon-person-profile-concept-isolated-on-white-background-with-social-member.jpg?b=1&s=170667a&w=0&k=20&c=X-ZNF80_ASKvWpbOICHC8Pf27CFCRyh7Ce9SDVopjZg=" alt="avatar">
                       </c:if> 
                     ...
                     <c:if test="${sessionScope.ROLE eq 'Admin' }">
                      ...
                      <div>
                        ...
                        <p class="text-xs text-slate-400 dark:text-navy-300">
                            <c:if test="${sessionScope.ROLE eq '2' }">Teacher</c:if>  
                           <c:if test="${sessionScope.ROLE eq '1' }">Studnet</c:if> 
                           <c:if test="${sessionScope.ROLE eq 'Admin' }">Admin</c:if> 
                        </p>
                      </div>
                    </div>
...
<div x-data="usePopper({placement:'right-end',offset:12})" @click.outside="isShowPopper &amp;&amp; (isShowPopper = false)" class="flex">
            ...
            <div class="flex items-center justify-between">
              <h2 class="text-base font-medium tracking-wide text-slate-700 line-clamp-1 dark:text-navy-100">
                ...
                <div class="flex items-center" >
                  <a href="<%=request.getContextPath() %>/add-new-teacher" class="btn bg-info font-medium text-white hover:bg-info-focus focus:bg-info-focus active:bg-info-focus/90">
                 <i class="fa-solid fa-plus"> </i> &nbsp; Add New Teacher
                </a> 
                </div>
              </div>
            </div>
		     ...
		     <div class="card invoice-list-wrapper">
		      <div class="card-datatable table-responsive">
		       <div class="dt-buttons" style="height : 0.5em;">
                ...
                <table class="invoice-list-table table is-zebra w-full text-left" id="teachersTable">
                  <thead>
                    <tr>
    ...
    <script src="/courses/resources/vendors/js/extensions/moment.min.js"></script>
    <script src="/courses/resources/vendors/js/tables/datatable/jquery.dataTables.min.js"></script>

     <script src="/courses/resources/js/datetime-moment-v1.10.12.js"  ></script>
       <script src="/courses/resources/vendors/js/tables/datatable/jquery.dataTables.min.js"></script>
    <script src="/courses/resources/vendors/js/tables/datatable/datatables.buttons.min.js"></script>
    <script src="/courses/resources/vendors/js/tables/datatable/dataTables.responsive.min.js"></script>
     ...
     <script src="/courses/resources/js/datetime-moment-v1.10.12.js"  ></script>
      ...
      <script src="/courses/resources/js/moment-v2.8.4.min.js"  ></script>
          <script src="/courses/resources/js/scripts/pages/app-invoice-list.min.js"></script>
    ...
    <script>
      ...
      function getUser(user_id){
        	$("#user_id").val(user_id);
        	$('#getForm').attr('action', '<%=request.getContextPath()%>/update-teacher');
        	$('#getForm').submit();
        }
      function getTeachersList(){
			...
			table.state.clear();		
		 	var myParams = {};
			$.ajax({url : "<%=request.getContextPath()%>/ajax/getTeachersList",type:"POST",data:myParams,success : function(data){    				
					if(data != null && data != '' && data.length > 0){    					
		         		$.each(data,function(key,val){
		         			var user_id = "'"+val.user_id+"'";
		                    var actions = '<a href="javascript:void(0);"  onclick="getUser('+user_id+');" class="btn btn-primary"  title="Edit"><i class="fa fa-pencil"></i></a>';    	                   	
		                   	var rowArray = [];    	                 
		            		var status = val.status;
		            		var approved_by_admin = $.trim(val.approved_by_admin);
		                	if(approved_by_admin == 'pending' || approved_by_admin == 'Pending'){status = '<sapn class="badge rounded-full bg-error text-white">'+approved_by_admin+'</span>'}
		                   	rowArray.push($.trim(val.id));
		                	rowArray.push("["+ $.trim(val.user_id)+"]"+" - "+ val.user_name);
		                	rowArray.push($.trim(val.phone));
		                	rowArray.push($.trim(val.email_id));
		                	rowArray.push(status);
		                	if('${sessionScope.ROLE}' == 'Admin'){rowArray.push($.trim(actions)); }
