...
<head>
    ...
    <title>
	     <c:if test="${action eq 'edit'}">Admin Center </c:if>
		 <c:if test="${action eq 'add'}"> Admin Center</c:if>   
   </title>
      ...
      <form id="addCourseForm" name="addCourseForm" class="row gy-1 pt-75" action="<%=request.getContextPath() %>/update-course-time-slots" method="post" class="form-horizontal" role="form">
        ...
        <div class="grid grid-cols-12 gap-4 sm:gap-5 lg:gap-6">
          <div class="col-span-12 lg:col-span-12">
            <div class="card">
               <c:forEach   var="obj" items="${coursessForStudentList}" varStatus="index">
                ...
                <div class="tab-content p-4 sm:p-5">
                  <div class="space-y-5">
                   ...
                   <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div class="flex">
                      <div class="flex h-20 w-full items-center justify-center rounded-lg bg-slate-200 dark:bg-navy-500 p-2">
                <p class="" style=" width: 24rem;">  Faculty : <i class="fa-solid fa-chalkboard-user"></i> <b>${obj.user_name } </b></p>
              </div>
      		
               <div class="mx-4 my-1 w-px bg-slate-200 dark:bg-navy-500"></div>
            <div class="flex h-20 w-full items-center justify-center rounded-lg bg-slate-200 dark:bg-navy-500 text-center">
                <p class="" style="width: 8rem;">  <i class="fa-solid fa-clock"></i> <b>${obj.hours } Hours</b> </p>
              </div>
               <div class="mx-4 my-1 w-px bg-slate-200 dark:bg-navy-500"></div>
              <div class="flex h-20 w-full items-center justify-center rounded-lg bg-slate-200 dark:bg-navy-500 text-center">
                <p class="" style="width: 8rem;"> Discount : <b>${obj.discount } <i class="fa fa-percent" aria-hidden="true"></i></b> </p>
              </div>
               <div class="mx-4 my-1 w-px bg-slate-200 dark:bg-navy-500"></div>
              <div class="flex h-20 w-full items-center justify-center rounded-lg bg-slate-200 dark:bg-navy-500 text-center">
                <p class="" style=" width: 12rem;">  Final Amount : <b>${obj.price_after_discount } Rs</b> </p>
              </div>
            </div>
                    ...
                    <div class="card px-4 pb-4 sm:px-5">
			            <div class="my-3 flex h-8 items-center justify-between">
			                <c:if test="${empty obj.datesList }">
			                No Dates Available!
			                </c:if>
			                <c:if test="${not empty obj.datesList }">
			            ...
			            <div class="card px-4 pb-4 sm:px-5">
			             <c:if test="${empty obj.datesList }">
			              ...
			              <div class="mt-5 grid grid-cols-1 place-items-start gap-6 sm:grid-cols-1" id="checkboxID">
			               <div class="card px-4 pb-4 sm:px-5" id="slotsSpace" style=" width: 64rem;">
			   				<c:forEach var="obj1" items="${obj.datesList}">
					                 <c:set var="string11" value="${obj1.slot_date }" />
									 <c:set var="string21" value="${fn:split(string11, '_')}" />
											<div class="row text-center mt-4">
					                
					                  <label class="inline-flex items-center space-x-2">
					                  <input id="${obj1.slot_dateOG }" value="${obj1.slot_dateOG }" <c:if test="${fn:contains(obj1.slot_date, 'checked')}"> checked  </c:if>
					                   onchange="setPriceMax('${obj.slot_dateOG }');getSLotsForDate('${string21[0]}','${obj1.slot_dateOG }','${obj1.course_id }');" class="form-checkbox forcheckbox is-basic h-5 w-5 rounded border-slate-400/70 checked:!border-success checked:bg-success hover:!border-success focus:!border-success dark:border-navy-400" type="checkbox">
					                  <span><b>${obj1.slot_dateOG }</b></span>
					                </label>
					                </div>
					                	
							            <div id="${obj1.slot_dateOG }div"><div class="my-3 flex h-8 items-center justify-between rounded-lg bg-gradient-to-r from-green-400 to-fuchsia-400 p-1">
								            <h2 class="font-medium tracking-wide text-slate-700 line-clamp-1 dark:text-navy-100 lg:text-base">Available Slots on - ${obj1.slot_dateOG } </h2>
								            </div><div class="card px-4 pb-4 sm:px-5">
								            <div class="mt-2 grid grid-cols-2 place-items-start gap-6 sm:grid-cols-3" id="${obj1.slot_dateOG }slot">
								            <c:forEach var="obj11" items="${obj1.slotsList}">
								            <label class="inline-flex items-center space-x-2">
								            <c:set var="string1" value="${obj11.time_slot }" />
											<c:set var="string2" value="${fn:split(string1, '_')}" />
											
								            <input id="0${obj1.slot_dateOG }timeSlot" <c:if test="${fn:contains(obj11.time_slot, 'checked')}"> checked  </c:if> onchange="setPricePerSLot('${obj1.slot_dateOG }','${index.count}${obj1.slot_dateOG }timeSlot');addToCArt('${obj1.slot_dateOG }');"
								             value="7 - 8 AM" class="form-checkbox slotCheckbox is-basic h-5 w-5 rounded border-slate-400/70 checked:!border-success checked:bg-success hover:!border-success 
								             focus:!border-success dark:border-navy-400" type="checkbox">
								            <span>${string2[0]}</span>
								            </label></c:forEach>
								             
								            </div>
								            <div style="text-align: end;">
							                   <button class="btn bg-success font-medium text-white hover:bg-success-focus focus:bg-success-focus active:bg-success-focus/90"
							                   onclick ="sendLink();">
									                  Send Link
									                </button>
								             ...
								             <input type="hidden" id="${obj1.slot_dateOG }Val" name="time_and_dates" value="">
