     ...
     <jsp:include page="../views/layout/header.jsp"></jsp:include> 
      ...
      <main class="main-content w-full px-[var(--margin-x)] pb-8">
        <div class="flex flex-col items-center justify-between space-y-4 py-5 sm:flex-row sm:space-y-0 lg:py-6">
         ...
         <c:if test="${action eq 'edit'}">
            <form id="addCourseForm" name="addCourseForm" class="row gy-1 pt-75" action="<%=request.getContextPath() %>/update-course" method="post" class="form-horizontal" role="form">
          </c:if>
          <c:if test="${action eq 'add'}">
            <form id="addCourseForm" name="addCourseForm" class="row gy-1 pt-75" action="<%=request.getContextPath() %>/add-course" method="post" class="form-horizontal" role="form">
 		</c:if>
              ...
              <div class="tabs flex flex-col">
                  ...
                  <div class="border-b-2 border-slate-150 dark:border-navy-500">
                    <div class="tabs-list -mb-0.5 flex">
                      <button class="btn h-14 shrink-0 space-x-2 rounded-none border-b-2 border-primary px-4 font-medium text-primary dark:border-accent dark:text-accent-light sm:px-5">
                        ...
                        <span>${courseDetails.course_name } Details</span>
                      </button>
                  ...
                  <div class="space-y-5">
                    ...
                    <span class="font-medium text-slate-600 dark:text-navy-100"></span>
                      ...
                      class="form-input w-full rounded-lg bg-slate-150 px-3 py-2 ring-primary/50 placeholder:text-slate-400 hover:bg-slate-200 focus:ring dark:bg-navy-900/90 dark:ring-accent/50 dark:placeholder:text-navy-300 dark:hover:bg-navy-900 dark:focus:bg-navy-900"
                       placeholder="Enter Course title" type="text" value="${courseDetails.course_name }">
                  	<span id="course_nameError" class="error-msg" ></span>
                  </label>
                  <label class="mt-1 flex -space-x-px">
                    <span class="font-medium text-slate-600 dark:text-navy-100"></span>
                      <input name="hours" id="hours"   min="0" max="300"
                      class="form-input w-full rounded-lg bg-slate-150 px-3 py-2 ring-primary/50 placeholder:text-slate-400 hover:bg-slate-200 focus:ring dark:bg-navy-900/90 dark:ring-accent/50 dark:placeholder:text-navy-300 dark:hover:bg-navy-900 dark:focus:bg-navy-900"
                       placeholder="Enter Hours of Course" type="number" value="${courseDetails.hours }">
                        <span class="flex items-center justify-center rounded-r-lg border border-slate-300 px-3.5 font-inter dark:border-navy-450">
                      <span>Hours</span>
                    </span> 
                  	<span id="hoursError" class="error-msg" ></span>
                  </label>
                   ...
                   <%--  <label class="block">
                      ...
                      <input name="course_name" id="course_name" 
                      class="form-input w-full rounded-lg bg-slate-150 px-3 py-2 ring-primary/50 placeholder:text-slate-400 hover:bg-slate-200 focus:ring dark:bg-navy-900/90 dark:ring-accent/50 dark:placeholder:text-navy-300 dark:hover:bg-navy-900 dark:focus:bg-navy-900"
                       placeholder="Enter post title" type="text" value="${courseDetails.course_name }">
                  	<span id="course_nameError" class="error-msg" ></span>
                    </label> --%>
                    <!-- <label class="block">
                      <span class="font-medium text-slate-600 dark:text-navy-100">Caption</span>
                      <input class="form-input w-full rounded-lg bg-slate-150 px-3 py-2 ring-primary/50 placeholder:text-slate-400 hover:bg-slate-200 focus:ring dark:bg-navy-900/90 dark:ring-accent/50 dark:placeholder:text-navy-300 dark:hover:bg-navy-900 dark:focus:bg-navy-900" type="text">
                    </label> -->
                    <div>
                      ...
                      <label class="block">
					    ...
					    >${courseDetails.description }</textarea>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                   <label class="mt-1 flex -space-x-px">
                    ...
                    <input class="form-input w-full border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:z-10 hover:border-slate-400 
                    focus:z-10 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent" placeholder="Enter Price in Rupees" 
                    id="course_cost"
                   name="course_cost"
                     placeholder="00"
                    type="number"  value="${courseDetails.course_cost }" >
                    <span class="flex items-center justify-center rounded-r-lg border border-slate-300 px-3.5 font-inter dark:border-navy-450">
                      <span>.00</span>
                    </span>
                  ...
                  <span class="relative mt-1.5 flex">
                    <input
                      class="form-input peer w-full  border border-slate-300 bg-transparent px-3 py-2  placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                      placeholder="% discount"
                      type="number" min="0" max="100"
                       name="discount"  
                       id="discount" value="${courseDetails.discount }"
                    />
                     <span class="flex items-center justify-center rounded-r-lg border border-slate-300 px-3.5 font-inter dark:border-navy-450">
                      <span>%</span>
                    </span>
                 ...
                 <span id="course_costError" class="error-msg" ></span>
                </div>
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                   <label class="mt-1 text-right">
                   <span></span>
                  </label>
                   <label class="mt-1 text-right">
                  <span class=" rounded-l-lg  font-inter dark:border-navy-450">
                    <b>Final Price :&nbsp;</b> <b><span id="final-price"><b>
						<c:if test="${not empty courseDetails.price_after_discount }" >
						 ${courseDetails.price_after_discount }
						</c:if>  
						<c:if test="${ empty courseDetails.price_after_discount }" >
						  0.00
						</c:if>                   
                  </b>
                  ...
                  <span class="relative mt-1.5 flex"> 
                      ...
                      <span class="font-medium text-slate-600 dark:text-navy-100"></span>
                     <input type="hidden" id="finalPrice"   name="price_after_discount" value="${courseDetails.price_after_discount }"/>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <input type="hidden" name="course_id" value="${courseDetails.course_id}" />
        </form>
	    ...
	    var validator =	$('#addCourseForm').validate({
	    	 ...
	    	 ignore: ":hidden:not(.select2 form-select)",
	       		...
	       		errorPlacement:function(error, element){
	    			...
	    			}else if(element.attr("id") == "hours" ){
	    				document.getElementById("hoursError").innerHTML="";
	    			 	error.appendTo('#hoursError');
	    			}else{
