...
<head>
    ...
    <title>PurpleWest - Courses</title>
...
<div x-data="usePopper({placement:'right-end',offset:12})" @click.outside="isShowPopper &amp;&amp; (isShowPopper = false)" class="flex">
            ...
            <div class="flex items-center justify-between">
              ...
              <div class="flex">
                <div class="flex items-center" >
                  <a href="<%=request.getContextPath() %>/add-new-course" class="btn bg-info font-medium text-white hover:bg-info-focus focus:bg-info-focus active:bg-info-focus/90">
    ...
    <script>
      ...
      function getCourse(course_id){
      	...
      	$('#getForm').attr('action', '<%=request.getContextPath()%>/update-course-form');
      	$('#getForm').submit();
      }
      ...
      function getCoursesList(){
			...
			table.state.clear();		
		 	var myParams = {};
			$.ajax({url : "<%=request.getContextPath()%>/ajax/getCoursesList",type:"POST",data:myParams,success : function(data){    				
					if(data != null && data != '' && data.length > 0){    					
		         		$.each(data,function(key,val){
		         			var course_id = "'"+val.course_id+"'";
		                    var actions = '<a href="javascript:void(0);"  onclick="getCourse('+course_id+');" class="btn btn-primary"  title="Edit"><i class="fa fa-pencil"></i></a>';    	                   	
		                   	var rowArray = [];    	                 
		            		
		                   	rowArray.push($.trim(val.id));
		                	rowArray.push($.trim(val.course_name));
		                	var faculty = $.trim(val.course_faculty);
		                	if(faculty == ''){faculty = 'No Teacher Added!'}else{faculty = "["+ $.trim(val.course_faculty)+"]"+" - "+ val.user_name}
		                	rowArray.push(faculty);
