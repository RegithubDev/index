...
.parent1 {
	...
	/* Optional, to center the items horizontally */
	grid-gap: 10px; /* Add some spacing between the items */
}

.child1 {
	background-color: #f0f0f0;
	/* Optional, add background color to the items */
}
...
.childs {
	...
	width: 221px;
	/* Optional, set a fixed width for child divs if needed */
	height: 100px;
	/* Optional, set a fixed height for child divs if needed */
...
.containers {
	display: flex;
	justify-content: center;
	/* Optional, to center the spans horizontally */
}
...
<body x-data class="is-header-blur" x-bind="$store.global.documentBody">
	...
	<!-- Page Wrapper -->
	<div id="root"
		class="min-h-100vh flex grow bg-slate-50 dark:bg-navy-900" x-cloak>
		<!-- Sidebar -->
				...
				<div
					...
					<div class="flex pt-4">
						<a href="<%=request.getContextPath()%>/home"> <img
							...
							src="/index/resources/images/app-logo.svg" alt="logo" />
						</a>
					</div>
					<!-- Main Sections Links -->
					...
					<div class="flex flex-col items-center space-y-3 py-3">
						<!-- Settings -->
						<a href="<%=request.getContextPath()%>/departments-master"
							class="flex h-11 w-11 items-center justify-center rounded-lg outline-none transition-colors duration-200 hover:bg-primary/20 focus:bg-primary/20 active:bg-primary/25 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25">
							<svg class="h-7 w-7" viewBox="0 0 24 24" fill="none"
								xmlns="http://www.w3.org/2000/svg">
                  <path fill-opacity="0.3" fill="#e21e26"
									...
									<div
										...
										<div>
											<a href="#"
												...
												${sessionScope.USER_NAME } </a>
											<p class="text-xs text-slate-400 dark:text-navy-300">
												${sessionScope.BASE_ROLE }</p>
										</div>
									</div>
                        ...
                        <div>
                          ...
                          >
                            Your profile setting
										...
										<div class="mt-3 px-4">
											<button
												class="btn h-9 w-full g_id_signout space-x-2 bg-primary text-white hover:bg-primary-focus 
                  ...
                  <path stroke-linecap="round" stroke-linejoin="round"
					...
					<div class="flex h-[calc(100%-4.5rem)] grow flex-col">
						<div class="is-scrollbar-hidden grow overflow-y-auto">
							<div class="mt-2 px-4"></div>
							<div x-data="{expanded:true}">
								<div class="mt-4 flex items-center justify-between px-4">
									<span class="text-xs font-medium uppercase">Departments
									</span>
									<div class="-mr-1.5 flex">
										<button @click="expanded =! expanded"
											...
											<svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5"
												...
												viewBox="0 0 24 24" stroke="#e21e26" stroke-width="2">
                          <path stroke-linecap="round"
										...
										<c:forEach var="obj" items="${departmentsList }"
											...
											<li><a
												...
												href="<%=request.getContextPath() %>${obj.common_url}/${obj.department_code}/${obj.department_name}">
													<div class="h-2 w-2 mt-1.5 rounded-full bg-current "
														style="color: #e21e26;"></div> <span>${obj.department_name }</span>
											</a></li>
										</c:forEach>
									</ul>
								</div>
						...
						<div class="flex items-center spacex-3 p-4"></div>
					</div>
				</div>
			</div>
		</div>
			...
			<div class="fixed right-0 top-0 z-[151] h-full w-full sm:w-80">
				<div x-data="{activeTab:'tabHome'}"
					...
					<div class="flex items-center justify-between py-2 px-4">
						<p x-show="activeTab === 'tabHome'"
							class="flex shrink-0 items-center space-x-1.5">
							<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4"
								fill="none" viewBox="0 0 24 24" stroke="#e21e26">
                  <path stroke-linecap="round" stroke-linejoin="round"
									stroke-width="1.5"
						...
						<p x-show="activeTab === 'tabProjects'"
							class="flex shrink-0 items-center space-x-1.5">
							<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4"
								fill="none" viewBox="0 0 24 24" stroke="#e21e26"
								stroke-width="1.5">
                  <path stroke-linecap="round" stroke-linejoin="round"
						...
						<p x-show="activeTab === 'tabActivity'"
							class="flex shrink-0 items-center space-x-1.5">
							<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4"
								fill="none" viewBox="0 0 24 24" stroke="#e21e26"
								stroke-width="1.5">
					...
					<div x-show="activeTab === 'tabProjects'"
							...
							<div class="rounded-lg bg-slate-100 p-3 dark:bg-navy-600">
								<div class="flex justify-between space-x-1">
									...
									<svg xmlns="http://www.w3.org/2000/svg" stroke-width="1.5"
										class="h-5 w-5 text-re dark:text-accent" fill="none"
										viewBox="0 0 24 24" stroke="#e21e26">
                      <path stroke-linecap="round"
											stroke-linejoin="round"
											d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
						...
						<div
							...
							<div class="mt-3 flex items-center justify-between space-x-2">
								...
								<button
									...
									<svg xmlns="http://www.w3.org/2000/svg"
										class="h-5 w-5 rotate-45" fill="none" viewBox="0 0 24 24"
										stroke="#e21e26">
                      <path stroke-linecap="round"
											stroke-linejoin="round" stroke-width="2"
                      ...
                      <path stroke-linecap="round"
						...
						<div
							...
							<div class="flex items-center space-x-3">
								<img class="h-10 w-10 rounded-lg object-cover object-center"
									src="/index/resources/images/illustrations/nft.svg" alt="image" />
								<div>
									<p
										class="font-medium leading-snug text-slate-700 dark:text-navy-100">
										NFT Marketplace App</p>
									<p class="text-xs text-slate-400 dark:text-navy-300">
										Updated at 5 Sep</p>
								</div>
							</div>
                  ...
                  <path stroke-linecap="round" stroke-linejoin="round"
							...
							<div
								...
								<div
									class="is-active relative w-8/12 overflow-hidden rounded-full bg-secondary dark:bg-secondary-light"></div>
							</div>
								...
								<div class="timeline-item-content flex-1 pl-4">
									<div
										...
										<p
											...
											New job alert</p>
										<span class="text-xs text-slate-400 dark:text-navy-300">12
											minute ago</span>
									</div>
								...
								<div class="timeline-item-content flex-1 pl-4">
									<div
										...
										<p
											class="pb-2 font-medium leading-none text-slate-600 dark:text-navy-100 sm:pb-0">
											Images Added</p>
										<span class="text-xs text-slate-400 dark:text-navy-300">1
											hour ago</span>
									</div>
									<p class="py-1">Mores Clarke added new image gallery</p>
									<div class="mt-4 grid grid-cols-3 gap-3">
										<img class="rounded-lg"
											src="/index/resources/images/object/object-1.jpg" alt="image" />
										<img class="rounded-lg"
											src="/index/resources/images/object/object-2.jpg" alt="image" />
										<img class="rounded-lg"
											src="/index/resources/images/object/object-3.jpg" alt="image" />
										<img class="rounded-lg"
											src="/index/resources/images/object/object-4.jpg" alt="image" />
										<img class="rounded-lg"
											src="/index/resources/images/object/object-5.jpg" alt="image" />
										<img class="rounded-lg"
											src="/index/resources/images/object/object-6.jpg" alt="image" />
									</div>
									<div class="mt-4">
										<span class="font-medium text-slate-600 dark:text-navy-100">
											Category: </span> <a href="#"
								...
								<div class="timeline-item-content flex-1 pl-4">
									<div
										...
										<p
											class="pb-2 font-medium leading-none text-slate-600 dark:text-navy-100 sm:pb-0">
											Design Completed</p>
										<span class="text-xs text-slate-400 dark:text-navy-300">3
											hours ago</span>
									</div>
									<p class="py-1">Robert Nolan completed the design of the
										CRM application</p>
									<a href="#"
								...
								<div class="timeline-item-content flex-1 pl-4">
									<div
										...
										<p
											class="pb-2 font-medium leading-none text-slate-600 dark:text-navy-100 sm:pb-0">
											ER Diagram</p>
										<span class="text-xs text-slate-400 dark:text-navy-300">a
											day ago</span>
									</div>
					...
					<div
						...
						<div
							...
							<button @click="activeTab = 'tabHome'"
								...
								<svg x-show="activeTab === 'tabHome'"
									xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0"
									viewBox="0 0 20 20" fill="#e21e26">
                    <path
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round"
							...
							<button @click="activeTab = 'tabProjects'"
								...
								class="btn h-9 rounded-full py-0 px-4 hover:bg-slate-300/20 hover:text-re focus:bg-slate-300/20 focus:text-re active:bg-slate-300/25 dark:hover:bg-navy-300/20 dark:hover:text-accent dark:focus:bg-navy-300/20 dark:focus:text-accent dark:active:bg-navy-300/25">
								<svg x-show="activeTab === 'tabProjects'"
									xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0"
									viewBox="0 0 20 20" fill="#e21e26">
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round"
							...
							<button @click="activeTab = 'tabActivity'"
								...
								class="btn h-9 rounded-full py-0 px-4 hover:bg-slate-300/20 hover:text-re focus:bg-slate-300/20 focus:text-re active:bg-slate-300/25 dark:hover:bg-navy-300/20 dark:hover:text-accent dark:focus:bg-navy-300/20 dark:focus:text-accent dark:active:bg-navy-300/25">
								<svg x-show="activeTab ===  'tabActivity'"
									xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0"
									viewBox="0 0 20 20" fill="#e21e26">
                    <path fill-rule="evenodd"
              ...
              >
                ...
                >
                  ...
                  />
						...
						<!-- Main Searchbar -->
						<template x-if="$store.breakpoints.smAndUp"></template>
						...
						<button
							...
							<svg x-show="$store.global.isDarkModeEnabled"
								...
								class="h-6 w-6 text-amber-400" fill="#e21e26"
								viewBox="0 0 24 24">
                  <path
							...
							<svg xmlns="http://www.w3.org/2000/svg"
								...
								x-transition:enter-start="scale-75"
								x-transition:enter-end="scale-100 static"
								class="h-6 w-6 text-amber-400" viewBox="0 0 20 20"
								fill="#e21e26">
                  <path fill-rule="evenodd"
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round"
							...
							<div :class="isShowPopper &amp;&amp; 'show'" class="popper-root"
								x-ref="popperRoot"
								style="position: fixed; inset: 0px 0px auto auto; margin: 0px; transform: translate(-98px, 58px);"
									...
									<div
										...
										<div
											...
											<button @click="activeTab = 'tabAll'"
												...
												class="btn shrink-0 rounded-none border-b-2 px-3.5 py-2.5 border-primary dark:border-accent text-primary dark:text-accent-light">
												<span>All</span>
											</button>
										...
										<div x-show="activeTab === 'tabAll'"
											...
											<div class="flex items-center space-x-3">
												<div
													...
													<svg xmlns="http://www.w3.org/2000/svg"
														class="h-5 w-5 text-info" fill="none" viewBox="0 0 24 24"
			...
			<div
				...
				<button
					...
					<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
						stroke-width="1.5" viewBox="0 0 24 24" stroke="#e21e26">
              <path stroke-linecap="round" stroke-linejoin="round"
							d="M15 19l-7-7 7-7" />
            </svg>
				</button>
			...
			<div
				...
				<div
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round"
						...
						<p
							...
							Kanban</p>
					</a> <a href="dashboards-crm-analytics.html" class="w-14 text-center">
						<div class="avatar h-12 w-12">
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round"
						...
						<p
							class="mt-1.5 overflow-hidden text-ellipsis whitespace-nowrap text-xs text-slate-700 dark:text-navy-100">
							Analytics</p>
					</a> <a href="apps-chat.html" class="w-14 text-center">
						<div class="avatar h-12 w-12">
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round"
						...
						<p
							class="mt-1.5 overflow-hidden text-ellipsis whitespace-nowrap text-xs text-slate-700 dark:text-navy-100">
							Chat</p>
					</a> <a href="apps-filemanager.html" class="w-14 text-center">
						<div class="avatar h-12 w-12">
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round"
						...
						<p
							class="mt-1.5 overflow-hidden text-ellipsis whitespace-nowrap text-xs text-slate-700 dark:text-navy-100">
							Files</p>
					</a> <a href="dashboards-crypto-1.html" class="w-14 text-center">
						<div class="avatar h-12 w-12">
							<div class="is-initial rounded-full bg-secondary text-white">
								<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
									fill="none" viewBox="0 0 24 24" stroke="#e21e26"
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round"
						...
						<p
							class="mt-1.5 overflow-hidden text-ellipsis whitespace-nowrap text-xs text-slate-700 dark:text-navy-100">
							Crypto</p>
					</a> <a href="dashboards-banking-1.html" class="w-14 text-center">
						<div class="avatar h-12 w-12">
							<div
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round"
						...
						<p
							class="mt-1.5 overflow-hidden text-ellipsis whitespace-nowrap text-xs text-slate-700 dark:text-navy-100">
							Banking</p>
					</a> <a href="apps-todo.html" class="w-14 text-center">
						<div class="avatar h-12 w-12">
							<div class="is-initial rounded-full bg-info text-white">
								<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
									fill="none" viewBox="0 0 24 24" stroke="#e21e26"
                    ...
                    <path d="M3 13.2L7.23529 18L17.8235 6"
						...
						<p
							class="mt-1.5 overflow-hidden text-ellipsis whitespace-nowrap text-xs text-slate-700 dark:text-navy-100">
							Todo</p>
					</a> <a href="dashboards-orders.html" class="w-14 text-center">
						<div class="avatar h-12 w-12">
				...
				<div class="mt-1 font-inter font-medium">
					<a
						...
						href="apps-chat.html"> <svg xmlns="http://www.w3.org/2000/svg"
                ...
                <path stroke-linecap="round" stroke-linejoin="round"
								d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
              </svg> <span>Chat App</span>
					</a> <a
						class="group flex items-center space-x-2 px-2.5 py-2 tracking-wide outline-none transition-all hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100"
						href="apps-filemanager.html"> <svg
							xmlns="http://www.w3.org/2000/svg"
							class="h-4.5 w-4.5 text-slate-400 transition-colors group-hover:text-slate-500 group-focus:text-slate-500 dark:text-navy-300 dark:group-hover:text-navy-200 dark:group-focus:text-navy-200"
							fill="none" viewBox="0 0 24 24" stroke="#e21e26"
							stroke-width="1.5">
                <path stroke-linecap="round" stroke-linejoin="round"
								d="M3 15a4 4 0 004 4h9a5 5 0 10-.1-9.999 5.002 5.002 0 10-9.78 2.096A4.001 4.001 0 003 15z" />
              </svg> <span>File Manager App</span>
					</a> <a
						class="group flex items-center space-x-2 px-2.5 py-2 tracking-wide outline-none transition-all hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100"
						href="404-3.html"> <svg xmlns="http://www.w3.org/2000/svg"
							class="h-4.5 w-4.5 text-slate-400 transition-colors group-hover:text-slate-500 group-focus:text-slate-500 dark:text-navy-300 dark:group-hover:text-navy-200 dark:group-focus:text-navy-200"
							fill="none" viewBox="0 0 24 24" stroke="#e21e26"
							stroke-width="1.5">
                <path stroke-linecap="round" stroke-linejoin="round"
								d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
              </svg> <span>Email App</span>
					</a> <a
						class="group flex items-center space-x-2 px-2.5 py-2 tracking-wide outline-none transition-all hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100"
						href="apps-kanban.html"> <svg
							xmlns="http://www.w3.org/2000/svg"
							class="h-4.5 w-4.5 text-slate-400 transition-colors group-hover:text-slate-500 group-focus:text-slate-500 dark:text-navy-300 dark:group-hover:text-navy-200 dark:group-focus:text-navy-200"
							fill="none" viewBox="0 0 24 24" stroke="#e21e26"
							stroke-width="1.5">
                <path stroke-linecap="round" stroke-linejoin="round"
						...
						class="group flex items-center space-x-2 px-2.5 py-2 tracking-wide outline-none transition-all hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100"
						href="apps-todo.html"> <svg xmlns="http://www.w3.org/2000/svg"
							class="h-4.5 w-4.5 text-slate-400 transition-colors group-hover:text-slate-500 group-focus:text-slate-500 dark:text-navy-300 dark:group-hover:text-navy-200 dark:group-focus:text-navy-200"
							fill="none" viewBox="0 0 24 24" stroke="#e21e26"
							stroke-width="1.5">
                <path d="M3 13.2L7.23529 18L17.8235 6"
                ...
                <path d="M12.5293 18L20.9999 8.40002"
								stroke-linecap="round" stroke-linejoin="round" />
              </svg> <span>Todo App</span>
					</a> <a
						class="group flex items-center space-x-2 px-2.5 py-2 tracking-wide outline-none transition-all hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100"
						href="dashboards-crypto-2.html"> <svg
							xmlns="http://www.w3.org/2000/svg"
							class="h-4.5 w-4.5 text-slate-400 transition-colors group-hover:text-slate-500 group-focus:text-slate-500 dark:text-navy-300 dark:group-hover:text-navy-200 dark:group-focus:text-navy-200"
							fill="none" viewBox="0 0 24 24" stroke="#e21e26"
							stroke-width="1.5">
                <path stroke-linecap="round" stroke-linejoin="round"
								d="M15 9a2 2 0 10-4 0v5a2 2 0 01-2 2h6m-6-4h4m8 0a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg> <span>Crypto Dashboard</span>
					</a> <a
						class="group flex items-center space-x-2 px-2.5 py-2 tracking-wide outline-none transition-all hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100"
						href="dashboards-banking-2.html"> <svg
							xmlns="http://www.w3.org/2000/svg"
							class="h-4.5 w-4.5 text-slate-400 transition-colors group-hover:text-slate-500 group-focus:text-slate-500 dark:text-navy-300 dark:group-hover:text-navy-200 dark:group-focus:text-navy-200"
							fill="none" viewBox="0 0 24 24" stroke="#e21e26"
							stroke-width="1.5">
                <path stroke-linecap="round" stroke-linejoin="round"
								d="M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3" />
              </svg> <span>Banking Dashboard</span>
					</a> <a
						class="group flex items-center space-x-2 px-2.5 py-2 tracking-wide outline-none transition-all hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100"
						href="dashboards-crm-analytics.html"> <svg
							xmlns="http://www.w3.org/2000/svg"
							class="h-4.5 w-4.5 text-slate-400 transition-colors group-hover:text-slate-500 group-focus:text-slate-500 dark:text-navy-300 dark:group-hover:text-navy-200 dark:group-focus:text-navy-200"
							fill="none" viewBox="0 0 24 24" stroke="#e21e26"
							stroke-width="1.5">
                <path stroke-linecap="round" stroke-linejoin="round"
								d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
              </svg> <span>Analytics Dashboard</span>
					</a> <a
						class="group flex items-center space-x-2 px-2.5 py-2 tracking-wide outline-none transition-all hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100"
						href="dashboards-influencer.html"> <svg
							xmlns="http://www.w3.org/2000/svg"
							class="h-4.5 w-4.5 text-slate-400 transition-colors group-hover:text-slate-500 group-focus:text-slate-500 dark:text-navy-300 dark:group-hover:text-navy-200 dark:group-focus:text-navy-200"
							fill="none" viewBox="0 0 24 24" stroke="#e21e26"
							stroke-width="1.5">
              ...
              </svg> <span>Influencer Dashboard</span>
					</a>
				</div>
			</div>
		</div>
		...
		<main
			...
			<div class="flex items-center space-x-4 py-5 lg:py-6 p-6">
				<h2
					...
					<a href="<%=request.getContextPath()%>/dep-content">Department
						Content</a>
				</h2>
				...
				<ul class="hidden flex-wrap items-center space-x-2 sm:flex">
					<li class="flex items-center space-x-2"><a
						class="text-primary transition-colors hover:text-primary-focus dark:text-accent-light dark:hover:text-accent"
						href="<%=request.getContextPath()%>/settings">Master</a> <svg
							x-ignore="" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4"
							fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round"
			...
			<c:if test="${action eq 'edit'}">
				<form id="addDCForm" name="addDCForm" class="row gy-1 pt-75"
					action="<%=request.getContextPath()%>/update-dcform" method="post"
					class="form-horizontal" role="form" enctype="multipart/form-data">
			</c:if>
			<c:if test="${action eq 'add'}">
				<form id="addDCForm" name="addDCForm" class="row gy-1 pt-75"
					action="<%=request.getContextPath()%>/add-dcform" method="post"
					class="form-horizontal" role="form" enctype="multipart/form-data">
			</c:if>
					...
					<div class="card p-4 sm:p-5">
						<div class="mt-4 space-y-4">
							...
							<div class="grid grid-cols-1 gap-4 sm:grid-cols-1">
								<label class="block "> <span>company</span> <span
									class="requried">*</span> <select id="company_code"
									name="company_code" onchange="setSBUList()"
									...
									class="form-select mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:bg-navy-700 dark:hover:border-navy-400 dark:focus:border-accent">
										...
										<c:forEach var="obj" items="${companiesList}">
											<option value="${obj.company_code }"
												<c:if test="${DCFromDetails.company_code eq obj.company_code }">selected</c:if>>[${obj.company_code }]
												- ${obj.company_name }</option>

										</c:forEach>
								</select> <span id="company_codeError" class="requried"></span>
								</label> <label class="block"> <span>SBU</span> <span
									class="requried">*</span> <select onchange="setDeptList();"
									id="sbu_code" name="sbu_code"
									<c:if test="${action eq 'edit'}"> disabled </c:if>
									class="form-select mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:bg-navy-700 dark:hover:border-navy-400 dark:focus:border-accent">
										<option value="">Select SBU</option>
										<c:forEach var="obj" items="${sbuList}">
											<option value="${obj.sbu_code }"
												<c:if test="${DCFromDetails.sbu_code eq obj.sbu_code }">selected</c:if>>[${obj.sbu_code }]
												- ${obj.sbu_name }</option>
										</c:forEach>
								</select> <span id="sbu_codeError" class="requried"></span>
								</label> <label class="block"> <span>Department</span> <span
									class="requried">*</span> <select onchange="setCategoryList();"
									id="department_code"
									<c:if test="${action eq 'edit'}"> disabled </c:if>
									...
									class="form-select mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:bg-navy-700 dark:hover:border-navy-400 dark:focus:border-accent">
										...
										<c:forEach var="obj" items="${deptList}">
											<option value="${obj.department_code }"
												<c:if test="${DCFromDetails.department_code eq obj.department_code }">selected</c:if>>[${obj.department_code }]
												- ${obj.department_name }</option>
										</c:forEach>
								</select> <span id="department_codeError" class="requried"></span>
								</label> <label class="block"> <span>Category</span><span
									class="requried">*</span> <select
									onchange="setSubCategoryList();" id="category" name="category"
									<c:if test="${action eq 'edit'}"> disabled </c:if>
									class="form-select mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:bg-navy-700 dark:hover:border-navy-400 dark:focus:border-accent">
								...
								</label> <label class="block"> <span>Sub Category</span><span
									class="requried">*</span> <select id="sub_category"
									name="sub_category"
									<c:if test="${action eq 'edit'}"> disabled </c:if>
									class="form-select mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:bg-navy-700 dark:hover:border-navy-400 dark:focus:border-accent">
				...
				<div class="col-span-12 lg:col-span-8">
					<div class="card">
                      ...
                      dark:focus:border-accent"
							...
							</label> <label class="block mb-4"> <span> Icon</span><span
                      ...
                      focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
								...
								value="${DCFromDetails.title_icon}"> <span
								id="title_iconError" class="requried"></span>
							</label> <label class="block mb-4"> <span>Document Type</span><span
								class="requried">*</span> <select id="document_type"
								name="document_type"
								<c:if test="${action eq 'edit'}"> disabled </c:if>
								onchange="setDocType(this.value)" id="document_type"
								name="document_type"
								class="form-select mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:bg-navy-700 dark:hover:border-navy-400 dark:focus:border-accent">
									<option value="">Select Type</option>
									<option value="Document"
										<c:if test="${DCFromDetails.document_type eq 'Document' }">selected</c:if>>Document</option>
									<option value="Link"
										<c:if test="${DCFromDetails.document_type eq 'Link' }">selected</c:if>>Link</option>
									<option value="Gallery"
										<c:if test="${DCFromDetails.document_type eq 'Gallery' }">selected</c:if>>Gallery</option>
									<!--  <option value="Form">Form</option> -->
							</select> <span id="document_typeError" class="requried"></span>
							</label>
							<div class="">
								<c:if test="${not empty DCFromDetails.link }">
									<label class="block mb-4"> <span> Link</span><span
								...
								<div class="grid grid-cols-1 gap-4 mt-2 sm:grid-cols-1">
									<div class="" id="imgSec" name="Attachments"
										style="display: none">
										<label class="block mb-4"> <span class="requried">*</span>
											<input id="Attachments" name="mediaList" type="file"
											class="mt-2" accept="image/*" multiple /> <span
											id="AttachmentsError" class="requried"></span>
										<!--  onchange="validateImages()" -->
										</label>
									</div>

									<div class="" id="docSec" style="display: none">
										<label class="block mb-4"> <span>Upload
												Document<br>(Only doc, docx, xls, xlsx, pdf are
												accepted)
										</span> <span class="requried">*</span> <input id="Attachments"
									...
									<c:choose>
										<c:when
											...
											<c:set var="filesList"
												value="${fn:split(DCFromDetails.attachments, ',')}" />
										...
										<c:otherwise>
											<c:if
												test="${ not empty fn:trim(DCFromDetails.attachments) }">

												<div class="control-group" id="fields0">
													<div class="controls">
														<div class="entry input-group upload-input-group">
															<input type="hidden" id="docs"
																value="${DCFromDetails.attachments}" />
															<div class="inline-space mt-5 flex flex-wrap items-end">



																<a
																	href="<%=CommonConstants.FILE_SAVING_PATH_LOC%>${DCFromDetails.department_code }/${DCFromDetails.category }/${DCFromDetails.sub_category }/${DCFromDetails.attachments}"
																	class="btn h-9 w-9 bg-success p-0 font-medium text-white hover:bg-success-focus hover:shadow-lg hover:shadow-success/50 focus:bg-success-focus focus:shadow-lg focus:shadow-success/50 active:bg-success-focus/90">
																	<i class="fa fa-arrow-down"></i>
																</a> <a
																	class="btn font-medium text-primary hover:bg-primary/20 focus:bg-primary/20 active:bg-primary/25 dark:text-accent-light dark:hover:bg-accent-light/20 dark:focus:bg-accent-light/20 dark:active:bg-accent-light/25">
											...
											<c:if
												test="${fn:length(DCFromDetails.attachments) ne 0 &&  empty fn:trim(DCFromDetails.attachments) }">
												<div class="control-group" id="fields">
													<div class="controls">
														<div class="entry input-group upload-input-group">No
															Media Found</div>
													</div>
												</div>
											</c:if>
							...
							<div class="mt-4 space-y-4">
								...
								<div class="grid grid-cols-1 gap-4 sm:grid-cols-1">
									<div class="max-w-xl">
										<p>Description</p>
										<div class="mt-5">
											<label class="block mb-4"> <textarea
													id="description1" name="description" rows="4"
                  ...
                  focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent">${DCFromDetails.description}</textarea>
								...
								<c:if test="${action eq 'edit'}">
									<div class="grid grid-cols-1 gap-4 sm:grid-cols-1">
										<label class="block mb-4"> <span>Status</span> <select
											id="status" name="status"
											class="form-select mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:bg-navy-700 dark:hover:border-navy-400 dark:focus:border-accent">
												<option>Select Status</option>
												<option value="Active"
													<c:if test="${DCFromDetails.status eq 'Active' }">selected</c:if>>Active</option>
												<option value="Inactive"
													<c:if test="${DCFromDetails.status eq 'Inactive' }">selected</c:if>>Inactive</option>
										</select>
										</label>
									</div>
								</c:if>
						...
						<div
							...
							<div class="flex justify-center space-x-2">
								<a
									...
									href="<%=request.getContextPath() %>/dep-content"> Cancel </a>
		...
		<template x-teleport="#x-teleport-target">
			<div
				...
				<div
					...
					x-transition:leave-end="opacity-0"></div>
				<div
					class="scrollbar-sm relative flex max-w-lg2 flex-col overflow-y-auto rounded-lg  px-4  text-center transition-opacity duration-300 dark:bg-navy-700 sm:px-5"
					...
					<div class="flex h-8 items-center justify-between">
						<h2
							class="font-medium tracking-wide text-slate-700 line-clamp-1 dark:text-navy-100 lg:text-base"></h2>

						<button @click="showModal = !showModal"
							class="btn -mr-1.5 h-7 w-7 rounded-full p-0 hover:bg-slate-300/20 focus:bg-slate-300/20 active:bg-slate-300/25 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25"
							style="margin-bottom: -4rem; z-index: 1; margin-right: -0.7rem; background-color: white;">
	...
	<form action="<%=request.getContextPath()%>/logout" name="logoutForm"
    ...
    /*  function onYouTubeIframeAPIReady() {
       ...
       function setSubCategoryList(){
	        ...
	        if ($.trim(department_code) !="" && $.trim(category) !="" ) {
	        	...
	        	var myParams = { department_code: department_code, category : category};
	            $.ajax({
				   			   ...
				   			   document.getElementById("sbu_codeError").innerHTML="";
				   		 	   error.appendTo('#sbu_codeError');
					   			   ...
					   			   document.getElementById("department_codeError").innerHTML="";
						   			...
						   			}else if(element.attr("id") == "sub_category" ){
							   			   document.getElementById("sub_categoryError").innerHTML="";
							   		 	   error.appendTo('#sub_categoryError');
							   			}else if(element.attr("id") == "content_title" ){
								   			   document.getElementById("content_titleError").innerHTML="";
								   		 	   error.appendTo('#content_titleError');
								   			}else if(element.attr("id") == "title_icon" ){
									   			   document.getElementById("title_iconError").innerHTML="";
									   		 	   error.appendTo('#title_iconError');
									   			}else if(element.attr("id") == "document_type" ){
										   			   document.getElementById("document_typeError").innerHTML="";
										   		 	   error.appendTo('#document_typeError');
										   			}else if(element.attr("id") == "description1" ){
       ...
       function setDocType(type){
    	   if ($.trim(type) !=""  ) {
    		   if(type == 'Document'){
    			   $('#docSec').css('display','block');
    			   $('#imgSec').css('display','none');
    			   $('#inputSec').css('display','none');
    		   }else if(type == 'Link'){
    			   $('#docSec').css('display','none');
    			   $('#imgSec').css('display','none');
    			   $('#inputSec').css('display','block');
    		   }else if(type == 'Gallery'){
    			   $('#docSec').css('display','none');
    			   $('#imgSec').css('display','block');
    			   $('#inputSec').css('display','none');
    		   }
    	   }
       }
