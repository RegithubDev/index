...
public class RoleMappingDao {
	...
	private String getDocCode(RoleMapping cObj) throws Exception {
		...
		try{
			con = dataSource.getConnection();
			String contract_updateQry = " SELECT STRING_AGG(QUOTENAME(document_code,''''), ',') as  document_code FROM [safety_ims] where project_code = ? and department_code = ?"
					+ " and  '"+cObj.getSafety_type()+"' LIKE CONCAT('%',incident_type,'%') and status is null ";
			int i = 1;
			stmt = con.prepareStatement(contract_updateQry);
